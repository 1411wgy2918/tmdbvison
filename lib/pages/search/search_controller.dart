...
abstract class _SearchPageController with Store {
  ...
  Future<void> searchBangumi(String input, {String type = 'add'}) async {
    ...
    if (currentDataSource == DataSourceType.tmdb) {
      // TMDB 搜索
      final page = (bangumiList.length ~/ 20) + 1;
      result = await DataSourceResolverPool.search(
        query: keywords,
        page: page,
      );
    } else {
      // Bangumi 搜索
      result = await BangumiApi.bangumiSearch(keywords,
  ...
  void setNotShowWatchedBangumis(bool value) {
    ...
    _collectRepository.updateSearchNotShowWatchedBangumis(value);
  }

  @action
  void setNotShowAbandonedBangumis(bool value) {
    notShowAbandonedBangumis = value;
    _collectRepository.updateSearchNotShowAbandonedBangumis(value);
  }
